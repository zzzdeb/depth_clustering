#include "./savitsky_golay_smoothing.h"
...
Mat SavitskyGolaySmoothing::RepairDepth(const Mat& no_ground_image, int step,
...
Mat SavitskyGolaySmoothing::RepairDepth(const Mat& depth_image) {
...
Mat SavitskyGolaySmoothing::CreateAngleImage(const Mat& depth_image) {
...
Mat SavitskyGolaySmoothing::GetSavitskyGolayKernel(int window_size) const {
  if (window_size % 2 == 0) {
...
Mat SavitskyGolaySmoothing::GetUniformKernel(int window_size, int type) const {
  if (window_size % 2 == 0) {
    throw std::logic_error("only odd window size allowed");
  }
...
Mat SavitskyGolaySmoothing::ApplySavitskyGolaySmoothing(const Mat& image,
                                                    int window_size) {
...
Radians SavitskyGolaySmoothing::GetLineAngle(const Mat& depth_image, int col,
