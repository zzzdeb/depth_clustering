...
Mat TunnelGroundRemover::ZeroOutGround(const cv::Mat& image,
...
Mat TunnelGroundRemover::ZeroOutGroundBFS(const cv::Mat& image,
                                         const cv::Mat& angle_image,
...
Mat TunnelGroundRemover::RepairDepth(const Mat& no_ground_image, int step,
...
Mat TunnelGroundRemover::RepairDepth(const Mat& depth_image) {
...
Mat TunnelGroundRemover::CreateAngleImage(const Mat& depth_image) {
...
Mat TunnelGroundRemover::GetSavitskyGolayKernel(int window_size) const {
  if (window_size % 2 == 0) {
...
Mat TunnelGroundRemover::GetUniformKernel(int window_size, int type) const {
  if (window_size % 2 == 0) {
    throw std::logic_error("only odd window size allowed");
  }
...
Mat TunnelGroundRemover::ApplySavitskyGolaySmoothing(const Mat& image,
                                                    int window_size) {
...
Radians TunnelGroundRemover::GetLineAngle(const Mat& depth_image, int col,
