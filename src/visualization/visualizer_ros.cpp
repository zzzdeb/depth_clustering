...
void Visualizer::addNode(ros::NodeHandle* nh)
{
  nh_ = *nh;
  pub = nh_.advertise<sensor_msgs::PointCloud>("cloud_test_viz",1);
}
...
void Visualizer::DrawCloud(const Cloud& cloud) {
  ...
  cloud_.points.resize(cloud.size());
  int i = -1;
  for (const auto& point : cloud.points()) {
    i++;
    cloud_.points[i].x = point.x(), 
    cloud_.points[i].y = point.y(), 
    cloud_.points[i].z = point.z();
  }
  cloud_.header.stamp = ros::Time::now();
  cloud_.header.frame_id = "platform_laser_rotation/base/rotation_center";
  pub.publish(cloud_);
}
