...
#ifndef SRC_IMAGE_LABELERS_DIFF_HELPERS_GROUND_DIFF_H_
#define SRC_IMAGE_LABELERS_DIFF_HELPERS_GROUND_DIFF_H_
 ...
 */
class GroundDiff : public AbstractDiff {
 public:
  explicit GroundDiff(const cv::Mat* source_image)
  ...
  float DiffAt(const PixelCoord& from, const PixelCoord& to) const override {
    return (_source_image->at<float>(from.row, from.col) -
  ...
  */
  bool SatisfiesThreshold(float value, float threshold) const override {
    return value > -threshold && value < 1.5*threshold; //!!!
  }
};
}  // namespace depth_clustering
